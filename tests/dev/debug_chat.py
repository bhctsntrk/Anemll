...
def run_prefill(embed_model, ffn_models, input_ids, context_pos, context_length, batch_size=64, state=None, causal_mask=None):
    ...
    print(f"[DEBUG PREFILL] Starting prefill: context_pos={context_pos}, batch_size={batch_size}")
    ...
    while batch_pos < context_pos:
        ...
        print(f"[DEBUG PREFILL] Call #{prefill_call_count}: batch_pos={batch_pos}, batch_end={batch_end}, current_batch_size={current_batch_size}")
        ...
        print(f"[DEBUG PREFILL] Position IDs: {position_ids[:10].tolist()}...{position_ids[-5:].tolist()}")
    ...
    print(f"[DEBUG PREFILL] Completed prefill. Total calls: {prefill_call_count}, final context_pos: {context_pos}")
...
def generate_next_token(embed_model, ffn_models, lmhead_model, input_ids, pos, context_length, metadata, state=None, causal_mask=None, temperature=0.0):
    ...
    print(f"[DEBUG GENERATE] Starting generation: pos={pos}, input_ids.shape={input_ids.shape}")
    
    # Get current token
    current_token = input_ids[:, pos-1:pos]  # [1, 1]
    print(f"[DEBUG GENERATE] Current token position: {pos-1}, token: {current_token.item()}")
    ...
    print(f"[DEBUG GENERATE] Position ID for generation: {position_ids.item()}")
