...
def test_batch_prefill_single_token(model):
    ...
    causal_mask_data = make_causal_mask(CONTEXT_LENGTH, prompt_length-1)
    causal_mask = torch.tensor(causal_mask_data, dtype=torch.float16)
    
    # Batch prefill
    input_ids = torch.tensor([context_tokens], dtype=torch.long)   
    # Pad input_ids to CONTEXT_LENGTH with zeros
    input_ids = F.pad(input_ids, (0, CONTEXT_LENGTH - prompt_length), value=0)
    # Run batched prefill
    batch_pos = 0
    batch_size = 128
