...
def run_segmented_evaluation(args):
    """Run segmented evaluation for comparison with ANE"""
    ...
    # Parse segmentation arguments
    step_size = getattr(args, 'step', 100)
    worst_count = getattr(args, 'worst', 5)
    
    print(f"Running segmented MLX evaluation:")
    print(f"  Model: {args.model}")
    print(f"  Total samples: {total_samples}")
    print(f"  Segment size: {step_size}")
    print(f"  Reporting worst {worst_count} segments")
    print()
    ...
    # Run evaluation on each segment
    segment_results = []
    overall_correct = 0
    overall_total = 0
    
    for start_idx in range(0, total_samples, step_size):
        end_idx = min(start_idx + step_size, total_samples)
        segment_size = end_idx - start_idx
        
        print(f"Processing MLX segment [{start_idx} .. {end_idx-1}]")
        
        # Create segment dataset
        segment_data = dataset.select(range(start_idx, end_idx))
        
        # Create requests for this segment manually using simple objects
        class SimpleRequest:
        ...
        for item in segment_data:
            ...
            # Create requests for " no" and " yes" continuations
            requests.append(SimpleRequest(context, " no"))
            requests.append(SimpleRequest(context, " yes"))
        
        # Get loglikelihood scores
        results = lm.loglikelihood(requests)
        
        # Process results to calculate accuracy
        correct = 0
        for i, item in enumerate(segment_data):
            no_score = results[i * 2][0]      # " no" score 
            yes_score = results[i * 2 + 1][0] # " yes" score
        ...
        accuracy = correct / segment_size if segment_size > 0 else 0
        ...
        overall_total += segment_size
        
        print(f"  MLX: {accuracy:.4f} ({correct}/{segment_size})")
    
    # Calculate overall accuracy
    overall_accuracy = overall_correct / overall_total if overall_total > 0 else 0
    
    # Sort by accuracy to find worst segments
    segment_results.sort(key=lambda x: x['accuracy'])
    worst_segments = segment_results[:worst_count]
    best_segments = segment_results[-worst_count:][::-1]  # Reverse for highest first
    
    # Print results
    print(f"\n{'='*80}")
    print(f"MLX Segmented BoolQ Evaluation Results")
    print(f"{'='*80}")
    print(f"Overall MLX Accuracy: {overall_accuracy:.4f} ({overall_correct}/{overall_total})")
    print()
    print(f"Worst {worst_count} MLX segments by accuracy (lowest first):")
    for seg in worst_segments:
        print(f"{seg['start']}\t{seg['end']}\t{seg['accuracy']:.4f}")
    print()
    print(f"Best {worst_count} MLX segments by accuracy (highest first):")
    for seg in best_segments:
        print(f"{seg['start']}\t{seg['end']}\t{seg['accuracy']:.4f}")
    print(f"{'='*80}")
    
    return segment_results, overall_accuracy
...
def main():
    ...
    parser.add_argument("--step", type=int, default=100, help="Segment size for evaluation")
    parser.add_argument("--worst", type=int, default=5, help="Number of worst segments to report")
    parser.add_argument("--segmented", action="store_true", help="Run segmented evaluation")
