...
def run_gemma3_tests(model_name: str = "google/gemma-3-270m-it",
    ...
    Args:
        ...
        lut_bits: LUT bits for FFN (default: "6,4" = 6-bit, group size 4)
        lut_embeddings: LUT bits for embeddings (default: "6,4" = 6-bit, group size 4)
        lut_lmhead: LUT bits for LM head (default: "6,4" = 6-bit, group size 4)
        ...
        skip_dedup: If True, skip anemll-dedup weight deduplication
    ...
    print(f"LUT FFN: {lut_bits}")
    print(f"LUT Embeddings: {lut_embeddings}")
    print(f"LUT LM Head: {lut_lmhead}")
    print(f"Context: {context_length}")
    print(f"Batch: {batch_size}")
    print(f"Argmax: {use_argmax}")
    print(f"Dedup: {not skip_dedup}")
    print()
    ...
    cmd = [
        ...
        "--lut", str(lut_bits),
        "--lut-embeddings", str(lut_embeddings),
        "--lut-lmhead", str(lut_lmhead),
        "--prefix", "gemma3",
    ]
    ...
    # Skip anemll-dedup weight deduplication
    if skip_dedup:
        cmd.append("--skip-anemll-dedup")
...
def main():
    ...
    parser.add_argument("--lut", type=str, default="6,4",
                       help="LUT bits for FFN layers (default: 6,4 = 6-bit, group size 4)")
    parser.add_argument("--lut-embeddings", type=str, default="6,4",
                       help="LUT bits for embeddings (default: 6,4 = 6-bit, group size 4)")
    parser.add_argument("--lut-lmhead", type=str, default="6,4",
                       help="LUT bits for LM head (default: 6,4 = 6-bit, group size 4)")
    ...
    parser.add_argument("--skip-anemll-dedup", action="store_true",
                       help="Skip anemll-dedup weight deduplication")
