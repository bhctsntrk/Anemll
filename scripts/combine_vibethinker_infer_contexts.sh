...
Options:
  ...
  --infer-fn STR                Source infer function name (default: main)
  --prefill-fn STR              Source prefill function name (default: main)
  --infer-kind STR              Input infer chunk kind (default: auto; FFN preferred, else FFN_PF)
  ...
  --batch-size N                Batch size written to meta.yaml (default: 32)
  --split-lm-head N             split_lm_head in meta.yaml (default: 16)
...
if [[ ! "${NUM_CHUNKS}" =~ ^[0-9]+$ ]] || [[ ! "${BATCH_SIZE}" =~ ^[0-9]+$ ]] || [[ ! "${SPLIT_LM_HEAD}" =~ ^[0-9]+$ ]]; then
  echo "--num-chunks, --batch-size, --split-lm-head must be integers" >&2
  exit 1
fi
